...
int stepper2(int (*f) (double, gsl_vector *, gsl_vector *),
    ...
    while (x < xmax+h){
        ...
        while (err_val == 0){
            ...
            printf("step %d\n", step);
            printf("\t old h: %0.10e\n", h);
            get_scale(scale, range, tolerance);
            //printf("\t scale1: %0.10e\n",scale);
            ...
            err_val = check_err(tolerance, 1);
            attempts += 1;
            //printf("temp_out_vec after changing h: ");
            //print_vec(integration_vectors.temp_out_vec);
            if(attempts > 5){
                printf(">5 attempts to get the error below tolerance");
                return -2;
                }
